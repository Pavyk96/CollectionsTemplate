...
{
    /**
    Использую интерфейс Map<>, реализацию HashMap<>, алгоритм работает за O(1), так как операция поиска по ключу
     выполняется за O(1). Так как мы орпеделяем хеш функцию ключа и по ней ищем конкретный бакет, в котором лежит наше
     значение. В нашем случае, мы делаем одно только действие - поиск по ключу, следовательно метод работает за O(1)
     */
